...
def results # current controller for results
   if params[:search].present? #only capture parameters if something has been entered
     ...
     # puts "geocoded"
      ...
      @geocode_true = 0
      redirect_to :back, flash: {error: "We are having trouble understanding your address. Please enter a valid address."}
      # puts @geocode_true
     end
     ...
     if @resources.empty? # note if that the user input was geocoded but was not near any resources in the database
       # puts "no results"
       @geocode_true = -1
       redirect_to :back, flash: {error: "Sorry, but there are no NYC resources within one mile of your address. Please enter another address."}
       # puts @geocode_true
       end
